...
import teste.aular.utils.StackObj;
...
public class HotelController {
    ...
    StackObj stackObj = new StackObj(10);
        ...
        if (hotel != null) {
            if (!hotelRepository.existsByDocumentId(hotel.getDocumentId())) {
                ...
                if (!hotelRepository.existsByEmail(hotel.getEmail())) {
                    ...
                    stackObj.push(hotel.getHotelId());
    ...
    public ResponseEntity undoPostHotel() {
        if (stackObj.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Empty stack, cannot undo hotel registration"
            );
        }

        if (!hotelRepository.existsById(stackObj.peek())) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Stack hotelId not found when trying to unsubscribe"
            );
        }
        System.out.println(stackObj.peek());
        deactiveHotel(stackObj.pop());
    ...
    public ResponseEntity postAllFields(@RequestBody HotelAllFieldsResponse hotelAllFieldsResponse,
        ...
        if (!hotelRepository.existsById(hotelId)) {
            throw new ResponseStatusException(
                    ...
                    "hotelId not found when registering address and services"
            );
        }

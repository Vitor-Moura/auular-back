...
public class PartnerController {
    ...
    public ResponseEntity<Void> patchReport(@PathVariable int partnerId, @RequestBody byte[] newReport) {
        if (!partnerRepository.existsById(partnerId)) {
            ...
            if (newReport.length < 10 || newReport.length > 10 * 1024 * 1024) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Report size only between 10b and 10Mb"
                );
            }
            String textContent = new String(newReport);
            partnerRepository.setReport(partnerId, newReport);
        }
        return ResponseEntity.status(200).build();
    }
    ...
    public ResponseEntity<byte[]> getReport(@PathVariable int partnerId) {
        ...
        byte[] relatorio = partnerRepository.getReport(partnerId);
        return ResponseEntity.status(200).header(
                "content-disposition", "attachment; filename=" + nomeArquivo).body(relatorio);
    }
    ...
    public ResponseEntity<?> reportGenerate(@PathVariable Integer partnerId) {
        if (partnerRepository.existsById(partnerId)) {
            Partner partner = partnerRepository.findById(partnerId).get();
            String fileName = "/Users/vitormoura/Desktop/reportFiles/" +
                    partner.getName().replace(" ", "-") + "-Capaigns.txt";

            List<Campaign> listCampaigns = campaignRepository.findAllByPartnerPartnerId(partnerId);

            PartnerCampaignTxtFileService.CapaignsPartnerTxtGenerate(listCampaigns, fileName);
            return ResponseEntity.status(200).build();
        }
        return ResponseEntity.status(204).build();
    }
